defmodule SymphonyElixir.PrimeAgentAppServerTest do
  ...
  test "prime run_turn sends prompt before awaiting completion" do
    if match?({:win32, _}, :os.type()) do
      ...
      assert source =~ "send_prime_prompt(port, prompt"
      assert source =~ "await_prime_completion"
      {send_idx, _} = :binary.match(source, "send_prime_prompt(port, prompt")
      {await_idx, _} = :binary.match(source, "await_prime_completion")
      assert send_idx < await_idx, "send_prime_prompt must precede await_prime_completion"

      # On Windows real port I/O via WSL bash is unreliable; source-order check is sufficient.
      assert source =~ ":ok = send_prime_prompt(port, prompt, issue)"
    else
      test_root =
        Path.join(
          System.tmp_dir!(),
      ...
      try do
        ...
        patched =
          if String.contains?(workflow_content, "prime:\n  command:") do
            String.replace(
              workflow_content,
              "prime:\n  command:",
              "prime:\n  turn_timeout_ms: 800\n  command:"
  ...
  test "prime run_turn return tuple matches harness contract" do
    if match?({:win32, _}, :os.type()) do
      ...
      # verify return tuple shape via inspection
      assert source =~ ~s({:ok, %{result: result, session_id: session_id, thread_id: session_id, turn_id: "1"}})
    else
      test_root =
        Path.join(
          System.tmp_dir!(),
